...
void servermsappend( const char *name )
{
...
void disconnectmaster(int m)
{
    ...
    //lastupdatemaster[m] = 0;
}

void servermsremove( const char *name )
{
  loopv( mastername )
  {
...
{
    if(mastername[m].empty()) return ENET_SOCKET_NULL;
    ...
    {
        logline(ACLOG_WARNING, sock==ENET_SOCKET_NULL ? "[%s] could not open socket" : "[%s] could not connect", mastername[m].c_str());
...
{
    if(masterinpos[m] >= masterin[m].length()) return;
    ...
    while(end)
    {
        *end++ = '\0';
        ...
        if(!strncmp(input, "failreg", cmdlen))
            logline(ACLOG_WARNING, "[%s] master server registration failed: %s", args, mastername[m].c_str());
        ...
        {
            Lua::callHandler( LUA_ON_MASTER_SERVER_COMMAND, "ss", mastername[m].c_str(), input );
            if ( m == 0 ) processmasterinput(input, cmdlen, args);
            Lua::callHandler( LUA_ON_MASTER_SERVER_COMMAND_AFTER, "ss", mastername[m].c_str(), input );
        }
...
void updatemasterserver(int m, int millis, int port, bool fromLua = false)
{
    if(fromLua || (!lastupdatemaster[m] || ((millis-lastupdatemaster[m])>40*60*1000 && (interm || !totalclients))))
    {
        ...
        if(!mastername[m].empty()) requestmasterf(m, "regserv %d %s %d\n", port, servername[0] ? servername : "noname", AC_VERSION);
        lastupdatemaster[m] = millis ? millis : 1;
