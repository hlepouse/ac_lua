  ...
  if ( dp )
  {
	...
	while ( ( entry = readdir(dp) ) != NULL )
	{
	  ...
	  if ( match )
	  {
		...
		scripts[ numberOfScripts-1 ] = lua_open();
...
static LuaArg* prepareArgs( const char *arguments, va_list vl, int &argc )
{
  ...
  while (*arguments)
  {
    ...
    switch (*arguments++)
    {
      ...
      case 'o': // object(s); объекты, которые находятся на данный момент на вершине стека скрипта, переданного как аргумент
      {
...
static void parseArgs( int argc, const LuaArg *args, int &numberOfArguments, ... )
{
  ...
  for ( int i = 0; i < argc; ++i )
  {
    switch ( args[i].type )
    {
      ...
      case 'o': // объекты, которые находятся на данный момент на вершине стека скрипта, переданного как аргумент
      {
        int n = args[i].count;
        if ( n > 0 )
        {
...
int Lua::callHandler( const char *handler, int argc, const LuaArg *args )
{
  ...
  /* Исход прерываемого с помощью PLUGIN_BLOCK действия (имеет место лишь тогда, когда по меньшей мере один из обработчиков вернул PLUGIN_BLOCK):
   * true - действие выполнено в одном из вызываемых обработчиков, который вернул 2 результата: PLUGIN_BLOCK и значение исхода (true/false);
   * false - действие не выполнено ни в одном из вызываемых обработчиков.
   */
  ...
  int result = -PLUGIN_BLOCK; // результат работы обработчика
  ...
  ALL_SCRIPTS
  {
    if ( lua_isfunction( SCRIPT, lua_gettop( SCRIPT ) - numberOfArguments ) )
    {
      ...
      else
      {
        ...
        lua_pop( SCRIPT, 1 ); // текст ошибки
      }
      lua_pop( SCRIPT, 2 ); // обработчики ошибок
    }
    else lua_pop( SCRIPT, numberOfArguments + 1 /* функция */ + 2 /* обработчики ошибок */ );
  }
...
{
  // Циклично вызываем все функции-генераторы с данным именем; останавливаемся, как только получили первый результат.
...
LUA_FUNCTION (luanotice)
{
  ...
  lua_pushvalue( L, 1 ); // копируем объект, посылаемый в скрипты как первый аргумент функции onLuaNotice; выталкивается в методе callHandler
...
LUA_FUNCTION (setip) // копия sethostname
{
  /*
  lua_checkstack( L, 2 );
  if ( !lua_isnumber( L, 1 ) || !lua_isstring( L, 2 ) ) return 0;
...
LUA_FUNCTION (removems)
{
  lua_checkstack( L, 1 );
  if ( !lua_isstring( L, 1 ) ) return 0;
  servermsremove( lua_tostring( L, 1 ) );
  return 0;
}
