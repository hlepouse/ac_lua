...
struct servermaprot : serverconfigfile
{
    ...
    void read( bool fromLua = false )
    {
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_MAPROT_READ, "" );
        return;
    }
    ...
    virtual configset *get(int ccs) { return configsets.inrange(ccs) ? &configsets[ccs] : NULL; }
...
struct serveripblacklist : serverconfigfile
{
    ...
    void read( bool fromLua = false )
    {
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_BL_READ, "" );
    }
    ...
    bool check(enet_uint32 ip) // ip: network byte order
    {
        ...
        t.ur = 0;
        bool blacklisted = ipranges.search(&t, cmpipmatch) != NULL;
        bool *var = ( bool* ) Lua::getFakeVariable( LUA_YIELD_IS_BLACKLISTED, NULL, LUA_TBOOLEAN, "sb", iptoa( ip ), blacklisted );
        if ( var != NULL )
        {
          blacklisted = (bool) var[0];
          delete[] var;
        }
        return blacklisted;
    }
...
struct serverforbiddenlist : serverconfigfile
{
    ...
    bool canspeech(char *s, int cn)
    {
        bool result = true;
        for (int i=0; i<num; i++){
            if ( !findpattern(s,entries[i][0]) ) continue;
            else if ( entries[i][1][0] == '\0' || findpattern(s,entries[i][1]) ) { result = false; break; } /// Lua mod
        }
        bool *var = ( bool* ) Lua::getFakeVariable( LUA_YIELD_CAN_SPEECH, NULL, LUA_TBOOLEAN, "isb", cn, s, result );
        if ( var != NULL )
        {
          result = (bool) var[0];
          delete[] var;
...
struct serverpasswords : serverconfigfile
{
    ...
    void read( bool fromLua = false )
    {
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_PASSWORDS_READ, "" );
    }
