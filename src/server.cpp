...
/// Lua mod {
unsigned long long int Lua_totalSent = 0ull, Lua_totalReceived = 0ull;
unsigned int Lua_lastMeasured[2] = { 0u, 0u };
extern "C" {
...
void sendspawn(client *c, bool fromLua = false)
{
    ...
    if ( !fromLua && Lua::callHandler( LUA_ON_PLAYER_SPAWN, "i", c->clientnum ) == Lua::PLUGIN_BLOCK ) return;
    ...
    if ( !fromLua ) Lua::callHandler( LUA_ON_PLAYER_SPAWN_AFTER, "i", c->clientnum );
}
...
void flagaction(int flag, int action, int actor, bool fromLua = false)
{
    ...
    if ( !fromLua ) Lua::callHandler( LUA_ON_FLAG_ACTION, "iii", actor, action, flag );
}
...
void arenacheck()
{
    ...
    arenaround = gamemillis+5000;
    Lua::callHandler( LUA_ON_ARENA_WIN, "i", alive ? alive->clientnum : -1 );
...
bool spamdetect(client *cl, char *text) // checks doubled lines and average typing speed
{
    ...
    if(cl->saychars > (SPAMCHARPERMINUTE * SPAMCHARINTERVAL) / 60)
        spam = true;
    bool *var = ( bool* ) Lua::getFakeVariable( LUA_YIELD_IS_SPAM_DETECTED, NULL, LUA_TBOOLEAN, "isb", cl->clientnum, text, spam );
    if ( var != NULL )
    {
      spam = (bool) var[0];
      delete[] var;
...
void serverdamage(client *target, client *actor, int damage, int gun, bool gib, const vec &hitpush = vec(0, 0, 0), bool fromLua = false)
{
    ...
    {
        if ( !fromLua )
          if ( Lua::callHandler( LUA_ON_PLAYER_DEATH, "iibi", target->clientnum, actor->clientnum, gib, gun ) == Lua::PLUGIN_BLOCK ) return;
        ...
        if(isdedicated && actor->type == ST_TCPIP && tk)
        {
            ...
            {
                int res = -Lua::PLUGIN_BLOCK;
                if ( !fromLua ) res = Lua::callHandler( LUA_ON_PLAYER_BAN_THRESHOLD_REACH, "i", actor->clientnum );
                if ( res != Lua::PLUGIN_BLOCK )
                	addban(actor, DISC_AUTOBAN);
                     ...
                     ( actor->state.teamkills >= -scl.kickthreshold &&
                       ...
                       actor->state.frags < 4 * actor->state.teamkills ) )
                        {
                       		int res = -Lua::PLUGIN_BLOCK;
                			if ( !fromLua ) res = Lua::callHandler( LUA_ON_PLAYER_KICK_THRESHOLD_REACH, "i", actor->clientnum );
                			if ( res != Lua::PLUGIN_BLOCK )
                       			disconnect_client(actor->clientnum, DISC_AUTOKICK);
                   		}
        }
        if ( !fromLua ) Lua::callHandler( LUA_ON_PLAYER_DEATH_AFTER, "iibi", target->clientnum, actor->clientnum, gib, gun );
    } else if ( target!=actor && isteam(target->team, actor->team) ) check_ffire (target, actor, damage); // friendly fire counter
    if ( !fromLua ) Lua::callHandler( LUA_ON_PLAYER_GET_DAMAGE_AFTER, "iiiib", actor->clientnum, target->clientnum, damage, gun, gib );
}
...
bool updateclientteam(int cln, int newteam, int ftr)
{
    ...
    if(cl.team == newteam && ftr != FTR_AUTOTEAM) return true; // no change
    if ( ftr != FTR_AUTOTEAM && ftr != FTR_SILENTFORCE )
    {
      bool actionResult;
    ...
    int oldteam = cl.team;
    cl.team = newteam;
    Lua::callHandler( LUA_ON_PLAYER_TEAM_CHANGE_AFTER, "iii", cl.clientnum, oldteam, ftr );
    return true;
}
...
void startgame(const char *newname, int newmode, int newtime, bool notify, bool fromLua)
{
    ...
    else
    {
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_MAP_CHANGE, "si", smapname, smode );
    }
}
...
int getbantype(int cn)
{
	...
	else if(checkgban(c.peer->address.host))
	{
		bantype = BAN_MASTER;
		//return BAN_MASTER;
	}
	else if(ipblacklist.check(c.peer->address.host))
	{
		bantype = BAN_BLACKLIST;
	...
	else
	{
		loopv(bans)
		{
			...
			if(b.address.host == c.peer->address.host) { bantype = b.type; break;}
		}
	}
		
    /*bool *var = ( bool* ) Lua::getFakeVariable( LUA_YIELD_IS_BANNED, NULL, LUA_TBOOLEAN, "sb", iptoa( c.peer->address.host ), banned );
    if ( var != NULL )
    {
      banned = (bool) var[0];
      delete[] var;
    ...
    /*double *var = ( double* ) Lua::getFakeVariable( LUA_YIELD_SERVER_OPERATOR, NULL, LUA_TNUMBER, "i", op );
    if ( var != NULL )
    {
      op = (int) var[0];
      delete[] var;
    ...
    */
    ...
    double *var = ( double* ) Lua::getFakeVariable( LUA_YIELD_BAN_TYPE, NULL, LUA_TNUMBER, "i", iptoa( c.peer->address.host ), bantype );
    if ( var != NULL )
    {
      bantype = (int) var[0];
      delete[] var;
...
{
    int op = -1;
    loopv(clients) if(clients[i]->type!=ST_EMPTY && clients[i]->role > CR_DEFAULT) { op = i; break; }
...
struct voteinfo
{
    ...
    void end(int result, bool fromLua = false)
    {
        if(action && !action->isvalid()) result = VOTE_NO; // don't perform() invalid votes
        if ( !fromLua && Lua::callHandler( LUA_ON_VOTE_END, "iiisi", result, owner, type, text, num1, num2 ) == Lua::PLUGIN_BLOCK ) result = VOTE_NO;
        ...
        if ( !fromLua ) Lua::callHandler( LUA_ON_VOTE_END_AFTER, "iiisi", result, owner, type, text, num1, num2 );
    }
...
/// Lua mod
int Lua_voteResult = -1; bool Lua_endVoteInstantly = true;
void Lua_evaluateVote( bool forceend )
{
  if ( curvote ) curvote->evaluate( forceend );
}
void Lua_endVote( int result )
{
  if ( Lua_endVoteInstantly ) {
    if ( curvote ) curvote->end( result, true );
  } else Lua_voteResult = result;
}
...
bool svote(int sender, int vote, ENetPacket *msg) // true if the vote was placed successfully
{
    ...
    else
    {
        ...
        Lua::callHandler( LUA_ON_PLAYER_VOTE, "ii", sender, vote );
        curvote->evaluate();
        return true;
    }
}
...
bool scallvote(voteinfo *v, ENetPacket *msg) // true if a regular vote was called
{
    ...
    Lua_voteResult = -1; Lua_endVoteInstantly = false;
    double *var = ( double* ) Lua::getFakeVariable( LUA_ON_PLAYER_CALL_VOTE, NULL, LUA_TNUMBER, "iisii", v->owner, v->type, v->text, v->num1, v->num2, error );
    if ( var != NULL )
    {
      error = (int) var[0];
      delete[] var;
...
void changeclientrole(int client, int role, char *pwd, bool force)
{
    ...
    if(force || role == CR_DEFAULT || (role == CR_ADMIN && pwd && pwd[0] && passwords.check(clients[client]->name, pwd, clients[client]->salt, &pd) && !pd.denyadmin))
    {
        ...
        if ( Lua::callHandler( LUA_ON_PLAYER_ROLE_CHANGE, "iisib", client, role, pwd, pd.line, clients[client]->connecting ) == Lua::PLUGIN_BLOCK ) return;
        if(role > CR_DEFAULT)
        {
    ...
    Lua::callHandler( LUA_ON_PLAYER_ROLE_CHANGE_AFTER, "iisib", client, oldrole, pwd, pd.line, clients[client]->connecting );
}
...
void disconnect_client(int n, int reason, bool fromLua)
{
    ...
    if ( !fromLua ) Lua::callHandler( LUA_ON_PLAYER_DISCONNECT_AFTER, "ii", n, reason );
}
...
void tryauth(client *cl, const char *user)
{
    ...
    if(!requestmasterf(0, "reqauth %u %s\n", cl->authreq, cl->authname))
    {
        cl->authreq = 0;
...
void answerchallenge(client *cl, uint id, char *val)
{
    ...
    if(!requestmasterf(0, "confauth %u %s\n", id, val))
    {
        cl->authreq = 0;
        sendf(cl->clientnum, 1, "ris", SV_SERVMSG, "not connected to authentication server");
    }
...
void process(ENetPacket *packet, int sender, int chan)
{
    ...
    if(cl && !cl->isauthed)
    {
        ...
        Lua::callHandler( LUA_ON_PLAYER_CONNECT, "i", cl->clientnum );
    ...
    while((curmsg = p.length()) < p.maxlen)
    {
		...
		Lua::callHandler( LUA_ON_SERVER_PROCESS, "iii", cl->clientnum, chan, type );
        ...
        switch(type)
        {
            ...
            {
                int s = getint(p);
                if ( Lua::callHandler( LUA_ON_PLAYER_SAY_VOICE, "ii", cl->clientnum, s ) == Lua::PLUGIN_BLOCK ) break;
                /* spam filter */
                if ( servmillis > cl->mute ) // client is not muted
                {
                    ...
                    Lua::callHandler( LUA_ON_PLAYER_SAY_VOICE_AFTER, "ii", cl->clientnum, s );
                }
            }
            break;
            ...
            case SV_WEAPCHANGE:
            {
                ...
                Lua::callHandler( LUA_ON_PLAYER_WEAPON_CHANGE, "ii", cl->clientnum, gunselect );
            ...
            case SV_SWITCHNAME:
            {
                ...
                if(namechanged)
                {
                	logline(ACLOG_INFO,"[%s] %s changed name to %s", cl->hostname, cl->name, text);
                	Lua::callHandler( LUA_ON_PLAYER_NAME_CHANGE, "is", cl->clientnum, text );
                }
                copystring(cl->name, text, MAXNAMELEN+1);
                if(namechanged)
                {
            ...
            case SV_SPAWN:
            {
                ...
                cl->state.gunselect = gunselect;
                Lua::callHandler( LUA_ON_PLAYER_SPAWN_CLIENT_SIDE, "i", cl->clientnum );
                QUEUE_BUF(
                {
            ...
            case SV_SUICIDE:
            {
                if ( Lua::callHandler( LUA_ON_PLAYER_SUICIDE, "i", cl->clientnum ) == Lua::PLUGIN_BLOCK ) break;
            ...
            case SV_SHOOT:
            {
                ...
                Lua::callHandler( LUA_ON_PLAYER_SHOOT, "ii rrrt3", cl->clientnum, shot.shot.gun, shot.shot.to[0], shot.shot.to[1], shot.shot.to[2] );
                int hits = getint(p);
                int tcn = -1;
                loopk(hits)
                {
                ...
                Lua::callHandler( LUA_ON_PLAYER_SHOOT_AFTER, "ii rrrt3", cl->clientnum, shot.shot.gun, shot.shot.gun, shot.shot.to[0], shot.shot.to[1], shot.shot.to[2] );
                break;
            }
            ...
            case SV_RELOAD:
            {
                ...
                Lua::callHandler( LUA_ON_PLAYER_WEAPON_RELOAD, "ii", cl->clientnum, reload.reload.gun );
                break;
            }
...
//                 if (cl) disconnect_client(cl->clientnum, DISC_TAGT); // remove this in the future, when auth is complete
            ...
            case SV_CLIENTPING:
            {
                ...
                if(cl)
                {
                  ...
                  Lua::callHandler( LUA_ON_PLAYER_PING_UPDATE, "ii", cl->clientnum, oldping );
                }
                QUEUE_MSG;
                break;
            }
            ...
            case SV_POS:
            {
                ...
                Lua::callHandler( LUA_ON_PLAYER_STATE_UPDATE, "i", cl->clientnum );
                break;
            }
            ...
            case SV_SENDMAP:
            {
                ...
                Lua::callHandler( LUA_ON_PLAYER_SEND_MAP_AFTER, "siiiiii", sentmap, sender, revision, mapsize, cfgsize, cfgsizegz, error );
                break;
            }
            ...
            case SV_REMOVEMAP:
            {
                ...
                RemoveError error;
                if(cl->role < reqrole) error = RE_NOPERMISSION;
                else if(readonlymap(mp)) error = RE_READONLY;
                else if(mp == MAP_NOTFOUND) error = RE_NOTFOUND;
                else error = RE_NOERROR;

                double *var = ( double* ) Lua::getFakeVariable( LUA_ON_PLAYER_REMOVE_MAP, NULL, LUA_TNUMBER, "sii", rmmap, sender, error );
                if ( var != NULL )
                {
                  error = (RemoveError) (int) var[0];
                  delete[] var;
                }

                if ( error == RE_NOPERMISSION ) reject = "no permission";
                else if ( error == RE_READONLY ) reject = "map is readonly";
                else if ( error == RE_NOTFOUND ) reject = "map not found";
                else if ( error == RE_NOERROR )
                {
                ...
                Lua::callHandler( LUA_ON_PLAYER_REMOVE_MAP_AFTER, "sii", rmmap, sender, error);
                break;
            }
            ...
            case SV_CALLVOTE:
            {
                ...
                switch(vi->type)
                {
                    case SA_MAP:
                    {
                        ...
                        if (vi->gonext)
                        {
                            int ccs = mode ? maprot.next(false,false) : maprot.get_next(false);
            ...
            case SV_EXTENSION:
            {
				...
				else if ( !strcmp( ext, "lua::info" ) )
				{
				  getstring( text, p, n );
				  string info;
				  sprintf( info, "\f5(!) \fPLua mod for AssaultCube server. Version: \f5%.1f\fP.", Lua::version );
				  sendservmsg( info, sender );
				}
                // else if()
                ...
                Lua::callHandler( LUA_ON_EXTENSION, "sis", ext, sender, text );
...
void rereadcfgs( bool fromLua = false )
{
    if ( !fromLua ) if ( Lua::callHandler( LUA_ON_CONFIG_REREAD, "" ) == Lua::PLUGIN_BLOCK ) return;
    maprot.read( fromLua );
    ipblacklist.read( fromLua );
    nickblacklist.read();
    forbiddenlist.read();
    passwords.read( fromLua );
    if ( !fromLua ) Lua::callHandler( LUA_ON_CONFIG_REREAD_AFTER, "" );
}
...
void serverslice(uint timeout)   // main server update, called from cube main loop in sp, or dedicated server loop
{
	...
	Lua_totalSent += serverhost->totalSentData - Lua_lastMeasured[0];
    Lua_totalReceived += serverhost->totalReceivedData - Lua_lastMeasured[1];

    tmr_tick();
	Lua::callHandler( LUA_ON_LOOP, "" );
...
//        if(m_lms) lmscheck();
    ...
    if(forceintermission || ((smode>1 || (gamemode==0 && nonlocalclients)) && gamemillis-intermission_check_shear-diff>0 && (gamemillis-intermission_check_shear)/60000!=(gamemillis-intermission_check_shear-diff)/60000))
        checkintermission();
...
void initserver(bool dedicated, int argc, char **argv)
{
    ...
    logline( ACLOG_INFO, "<Lua> Using %s as Lua scripts directory", lua_scripts_path );
	Lua::initialize( lua_scripts_path );
    if ( hitreg_fixed ) logline( ACLOG_INFO, "Hitreg-fix enabled" );
    else logline( ACLOG_INFO, "Hitreg-fix disabled" );

    
    servermsappend( AC_MASTER_URI );
    extern int masterservers;
    ...
    Lua::callHandler( LUA_ON_INIT_AFTER, "" );
...
int main(int argc, char **argv)
{
    ...
    MUTEX_FREE( &sendpacketMutex );
    MUTEX_FREE( &writedemoMutex );
    return EXIT_SUCCESS;
